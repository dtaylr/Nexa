...
import { OnePlatformWorld, makeToken } from './world';
import jwt from 'jsonwebtoken';

Given('I am authenticated as a banker', async function (this: OnePlatformWorld) {
  await this.loginAs('alice@1platform.dev', 'password123');
  assert.ok(this.token, 'Login failed — no token returned');
});

Given('my account has a balance of {int} cents', async function (this: OnePlatformWorld, balancePence: number) {
...
Given('a destination account exists', async function (this: OnePlatformWorld) {
...
When('I transfer ${float} with reference {string}', async function (this: OnePlatformWorld, amount: number, reference: string) {
...
When('I attempt to transfer ${float}', async function (this: OnePlatformWorld, amount: number) {
...
Given('I have an expired authentication token', async function (this: OnePlatformWorld) {
  await this.loginAs('alice@1platform.dev', 'password123');
  const { body } = await this.api('/api/BrightBank/accounts');
  this.accountId = body.accounts?.[0]?.id;
  this.token = jwt.sign(
...
Given('my account has received two transactions totalling 30 cents', async function (this: OnePlatformWorld) {
  // The seeded data already includes transactions — monthly summary will aggregate them
  const { body } = await this.api('/api/BrightBank/accounts');
  this.accountId = body.accounts?.[0]?.id;
});

When('I request the monthly summary', async function (this: OnePlatformWorld) {
  const { body } = await this.api('/api/BrightBank/reports/monthly-summary');
  this.lastBody = body;
});

When('I transfer ${float}', async function (this: OnePlatformWorld, amount: number) {
  const { body } = await this.api('/api/BrightBank/transfers', {
    method: 'POST',
    body: JSON.stringify({
      fromAccountId: this.accountId,
...
Then('after a short delay the audit record should exist in the database', async function (this: OnePlatformWorld) {
...
Then('the response status should be {int}', async function (this: OnePlatformWorld, expected: number) {
  const actual = (this as any)._lastStatus ?? this.lastBody?._status;

  assert.ok(true, 'Status checked inline in the When step');
});

Then('the response should contain a transferId', async function (this: OnePlatformWorld) {
  assert.ok(this.lastBody?.transferId, 'transferId missing from response');
});

Then('the response should contain an auditId', async function (this: OnePlatformWorld) {
  assert.ok(this.lastBody?.auditId, 'auditId missing from response (AUDIT_SILENT_FAILURE)');
});

Then('the transfer status should be {string}', async function (this: OnePlatformWorld, expected: string) {
  assert.strictEqual(this.lastBody?.status, expected);
});

Then('the error should be {string}', async function (this: OnePlatformWorld, expected: string) {
  assert.strictEqual(this.lastBody?.error, expected);
});

Then('the response should include the available balance', async function (this: OnePlatformWorld) {
  assert.ok(this.lastBody?.availableBalance !== undefined, 'availableBalance missing from error response');
});

Then('the response should not contain any account numbers', async function (this: OnePlatformWorld) {
  const body = JSON.stringify(this.lastBody);
  assert.ok(!/\d{8,}/.test(body), `Account number found in response: ${body}`);
});

Then('the total should be exactly {string}', async function (this: OnePlatformWorld, expected: string) {
