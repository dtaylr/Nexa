...
Given('my patient record exists in the system', async function (this: NexaWorld) {
  const { body } = await this.api('/api/health/patients/1');
  this.patientRowId = body.id ?? 1;
});

When('I book an appointment with {string} for next week', async function (this: NexaWorld, doctorId: string) {
  const datetime = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString();
  const { body } = await this.api('/api/health/appointments', {
    method: 'POST',
...
Given('I have a scheduled appointment', async function (this: NexaWorld) {
...
When('I cancel that appointment', async function (this: NexaWorld) {
  const { body } = await this.api(`/api/health/appointments/${this.appointmentId}/cancel`, {
    method: 'PUT',
  });
  this.lastBody = body;
});

Then('the appointment status should be {string}', async function (this: NexaWorld, expected: string) {
  assert.strictEqual(this.lastBody?.status, expected);
});

Then('the appointment should be retrievable by patient ID', async function (this: NexaWorld) {
  const { body } = await this.api(`/api/health/patients/${this.patientRowId}/appointments`);
...
Given('my patient record has an active medication with dosage {string}', async function (this: NexaWorld, _dosage: string) {
  // Medication seeded in db/seed.ts — already exists for patient 1
});

When('I retrieve my medications', async function (this: NexaWorld) {
  const { body } = await this.api(`/api/health/patients/${this.patientRowId}/medications`);
  this.lastBody = body;
});

Then('the dosage value should be of type number', async function (this: NexaWorld) {
...
Given('another patient record exists with sequential ID', async function (this: NexaWorld) {
  // Patient 2 is seeded with ID 2 — sequential integer IDOR target
});

When('I attempt to access that patient\'s records using their ID', async function (this: NexaWorld) {
  const { body } = await this.api('/api/health/patients/2/records');
  this.lastBody = body;
});

When('I request a patient record with an invalid ID {string}', async function (this: NexaWorld, invalidId: string) {
  const { body } = await this.api(`/api/health/patients/${invalidId}`);
  this.lastBody = body;
});
...
When('two booking requests are submitted simultaneously for the same slot', async function (this: NexaWorld) {
  ...
  const [r1, r2] = await Promise.all([
    this.api('/api/health/appointments', { method: 'POST', body: JSON.stringify(payload) }),
    this.api('/api/health/appointments', { method: 'POST', body: JSON.stringify(payload) }),
  ]);
