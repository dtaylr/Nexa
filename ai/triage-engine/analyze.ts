...
function parseJUnitResults(filePath: string, domain: string): TestResult[] {
  ...
  while ((match = testcaseRegex.exec(content)) !== null) {
    ...
    results.push({
      domain,
      testName: name,
      status: failureMatch ? 'failed' : body.includes('<skipped') ? 'skipped' : 'passed',
...
function triage(results: TestResult[]): TriageResult[] {
  ...
  return triaged.sort((a, b) => SEVERITY_ORDER[a.severity] - SEVERITY_ORDER[b.severity]);
}
...
function report(triaged: TriageResult[], allResults: TestResult[]) {
  ...
  console.log('\n═══════════════════════════════════════════════════');
  console.log('  NexaCore — AI Triage Report');
  console.log('═══════════════════════════════════════════════════\n');
  ...
  const byDomain = triaged.reduce((acc, t) => {
    (acc[t.domain] = acc[t.domain] || []).push(t);
    return acc;
  }, {} as Record<string, TriageResult[]>);

  for (const [domain, items] of Object.entries(byDomain)) {
    console.log(`  ── ${domain.toUpperCase()} (${items.length} failure${items.length > 1 ? 's' : ''}) ──\n`);
    for (const item of items) {
      ...
      if (item.escalate) console.log(`     ⚠️  ESCALATE — block release pending review`);
      console.log();
    }
  }
...
for (const domain of domains) {
  const junitPath = path.join(process.cwd(), 'results', 'api', domain, 'junit.xml');
