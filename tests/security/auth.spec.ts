...
import { makeToken, makeExpiredToken, seedUser, seedAccount } from '../helpers';
...
describe('API1 — Broken Object Level Authorization (IDOR)', () => {
  it('patient cannot access another patient\'s records via ID enumeration', async () => {
    ...
    const res = await supertest(app)
      .get(`/api/health/patients/${targetPatientId}/records`)
      .set('Authorization', `Bearer ${attackerToken}`);

    expect(res.status, 'IDOR: patient accessed another patient\'s records (HLT-005)').toBe(403);
  });
  ...
  it('user cannot access another user\'s financial account', async () => {
    ...
    const res = await supertest(app)
      .get(`/api/finance/accounts/${u2Account}`)
      .set('Authorization', `Bearer ${attackerToken}`);
...
describe('API2 — Broken Authentication', () => {
  it('missing Authorization header returns 401', async () => {
    const [fin, hlt, com] = await Promise.all([
      supertest(app).get('/api/finance/accounts'),
      supertest(app).get('/api/health/patients/1'),
...
describe('Security Headers', () => {
  ...
  it('sensitive endpoints do not expose server version', async () => {
    const res = await supertest(app).get('/api/finance/accounts').set('Authorization', 'Bearer bad');
