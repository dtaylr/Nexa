...
import { makeToken, seedUser, seedAccount } from '../helpers';
...
describe('Input Validation — Finance', () => {
  ...
  it('transfer with negative amount is rejected', async () => {
    const res = await supertest(app)
      ...
      .send({ fromAccountId: accountId, toAccountId: destId, amount: -100, currency: 'GBP' });
  ...
  it('transfer with zero amount is rejected', async () => {
    const res = await supertest(app)
      .post('/api/finance/transfers')
      .set('Authorization', `Bearer ${token}`)
      .send({ fromAccountId: accountId, toAccountId: destId, amount: 0, currency: 'GBP' });
    expect(res.status).toBe(400);
  });
  ...
  it('transfer with extremely large amount does not cause integer overflow', async () => {
    const res = await supertest(app)
      .post('/api/finance/transfers')
      .set('Authorization', `Bearer ${token}`)
      .send({ fromAccountId: accountId, toAccountId: destId, amount: Number.MAX_SAFE_INTEGER, currency: 'GBP' });
  ...
  it('transfer with non-numeric amount is rejected', async () => {
    const res = await supertest(app)
      .post('/api/finance/transfers')
      .set('Authorization', `Bearer ${token}`)
      .send({ fromAccountId: accountId, toAccountId: destId, amount: 'DROP TABLE fin_accounts;--', currency: 'GBP' });
    expect([400, 422]).toContain(res.status);
    expect(res.status).not.toBe(500);
  });
  ...
  it('SQL injection in account ID path does not cause 500', async () => {
    ...
    const res = await supertest(app)
      .get(`/api/finance/accounts/${encodeURIComponent(malicious)}`)
      .set('Authorization', `Bearer ${token}`);
    expect(res.status).not.toBe(500);
  });
});
...
describe('Input Validation — Commerce', () => {
  ...
  beforeAll(async () => {
    ...
    db.prepare(
      "INSERT INTO com_products (id, name, price, inventory) VALUES (?, ?, ?, ?)"
