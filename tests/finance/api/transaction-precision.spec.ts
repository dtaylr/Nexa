...
import { makeToken, makeExpiredToken, makeRecentlyExpiredToken, seedUser, seedAccount, getAccountBalance } from '../../helpers';
...
describe('Finance API — Precision & Concurrency', () => {
  ...
  it('FIN-002: monthly summary exposes floating-point arithmetic errors', async () => {
    ...
    // BUG FIN-002: this will likely be 0.30000000000000004 not 0.30
  ...
  it('FIN-001: audit record must exist for every successful transfer', async () => {
    const res = await supertest(app)
      .post('/api/finance/transfers')
      .set('Authorization', `Bearer ${aliceToken}`)
      .send({ fromAccountId: aliceAccountId, toAccountId: bobAccountId, amount: 10, currency: 'GBP', reference: 'test' });
  ...
  it('FIN-004: concurrent transfers cannot create a negative balance', async () => {
    ...
    const transfers = Array.from({ length: 10 }, () =>
      supertest(app)
        ...
        .send({ fromAccountId: accountId, toAccountId: targetId, amount: 20, currency: 'GBP' })
    );
  ...
  it('FIN-005: account ID must not appear in error responses', async () => {
    const nonExistentId = 'acc-does-not-exist-999';
    const res = await supertest(app)
      .get(`/api/finance/accounts/${nonExistentId}`)
      .set('Authorization', `Bearer ${aliceToken}`);

    expect(res.status).toBe(404);
    const body = JSON.stringify(res.body);
    expect(body, 'Account ID was leaked in error response (FIN-005)').not.toContain(nonExistentId);
  });

  it('FIN-003: expired tokens are rejected', async () => {
    const expiredToken = makeExpiredToken({ id: aliceId, email: 'alice@test.dev', role: 'banker' });
    const res = await supertest(app)
      .get(`/api/finance/accounts/${aliceAccountId}`)
      .set('Authorization', `Bearer ${expiredToken}`);

    expect(res.status).toBe(401);
  });

  it('FIN-003: recently expired tokens should be rejected but are accepted (grace period bug)', async () => {
    ...
    // BUG FIN-003: the API accepts this because of the 30-second grace period
    // This test documents the bug — it expects 401 but the implementation returns 200
    expect(res.status, 'BUG FIN-003: recently expired token should be rejected').toBe(401);
  });
  ...
  it('transfer with insufficient funds returns structured error without PII', async () => {
    const res = await supertest(app)
      .post('/api/finance/transfers')
      .set('Authorization', `Bearer ${aliceToken}`)
      .send({ fromAccountId: aliceAccountId, toAccountId: bobAccountId, amount: 500, currency: 'GBP' });
